...
func GetProfile(c *fiber.Ctx) error {
	...
	var id, phone, role, name, companyName, avatarEmoji, language string
	var rating float64
	err := database.DB.QueryRow(context.Background(), `
		SELECT id, phone, role, name, company_name, avatar_emoji, language, rating
		FROM users WHERE id = $1
	`, userID).Scan(&id, &phone, &role, &name, &companyName, &avatarEmoji, &language, &rating)
	if err != nil {
		return c.Status(404).JSON(fiber.Map{"error": "user not found"})
	}
	return c.JSON(fiber.Map{
		"id": id, "phone": phone, "role": role, "name": name,
		"company_name": companyName, "avatar_emoji": avatarEmoji,
...
func UpdateProfile(c *fiber.Ctx) error {
	...
	_, err := database.DB.Exec(context.Background(), `
		...
		SET name = COALESCE(NULLIF($1,''), name),
		    ...
		    avatar_emoji = COALESCE(NULLIF($3,''), avatar_emoji),
		    language = COALESCE(NULLIF($4,''), language)
		WHERE id = $5
	`, body.Name, body.CompanyName, body.AvatarEmoji, body.Language, userID)
	if err != nil {
